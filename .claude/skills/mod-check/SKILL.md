...
- **XML syntax**: unclosed tags, mismatched attribute quotes, malformed UUIDs (`xxxxxxxx-xxxx-xxxx-xxxx-xxxxxxxxxxxx` format)
...
- **Syntax**: `new entry` / `type` / `data` / `using` structure is valid; no missing quotes or stray characters
...
- **Syntax**: parse errors, unclosed `do/end`, `if/end`, `function/end` blocks
...
- **Syntax**: well-formed XML, no unclosed tags
